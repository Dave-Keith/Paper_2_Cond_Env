...
base.cond.ts <- base.cond.ts %>% dplyr::filter(year < 2020) # Just go to 2019
dfo.sst <- read.csv("D:/Github/Paper_2_Cond_Env/Data/dfo_may_aug_sst_and_condition_time_series.csv")
...
sst.raw.dat <- read.csv("D:/Github/Paper_2_Cond_Env/Data/dfo_sst_ts.csv")
...
```{r, message=F, warning=F}
# For the sst data I need to massage it a bit...
sst.raw.dat <- sst.raw.dat %>% dplyr::select(year,month,GBS_weighted_median)
...
for(y in 2000:2018)
{
  ...
  # years and so that is all nicely accounted for in this I checked and even 2018 looks to be correct, so no
  # need to do anything here since we only run to 2018... if we had 2020 we'd probably need to get funky.
