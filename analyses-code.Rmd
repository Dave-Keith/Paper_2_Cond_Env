...
all.dat <- left_join(sst.dat,cond.dat,by='year')
...
# So this suggests it's better in that the etremes and 1st/3/rd quartiles are smaller.
# Also has the advantage that if we miss a survey we still have an estimate of condition.

# OK, so now let's get the different growth estimates from these...
...
laa.t <- vonB$Linf*(1-exp(-vonB$K)) + exp(-vonB$K) * mod.dat$l.bar
...
save_plot(paste0(direct.proj,"Results/Figures/center_of_gravity.tiff"),plt.cog,base_width =8,base_height =8,units='in')
save_plot(paste0(direct.proj,"Results/Figures/center_of_gravity.png"),plt.cog,base_width =8,base_height =8,units='in')
```
